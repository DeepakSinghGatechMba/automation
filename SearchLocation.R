...
wb = createWorkbook()
addWorksheet(wb, "SearchLocation") 
writeData(wb, sheet = "SearchLocation", SiteList) # Will default add from cell A1 
date=unlist(strsplit(as.character.Date(Sys.time()),":"))
saveWorkbook(wb,paste(CustomerName,"_SearchLocation",paste(date[1],"_",date[2],sep=""),".xlsx",sep = ""),overwrite = T) 
